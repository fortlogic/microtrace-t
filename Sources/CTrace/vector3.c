...
const vector3_t zero3 = {0.0, 0.0, 0.0};
const vector3_t unit3_i = {1.0, 0.0, 0.0};
const vector3_t unit3_j = {0.0, 1.0, 0.0};
const vector3_t unit3_k = {0.0, 0.0, 1.0};
...
vector3_t vec3_normalize(vector3_t vec) {
  vector3_t norm = vec3_scale(1.0 / vec3_length(vec), vec);
  return norm;
}
...
float vec3_distance(vector3_t vec1, vector3_t vec2) {
  float dist = fabsf(vec3_length(vec3_subtract(vec1, vec2)));
  return dist;
}
